...
# sex-specific ----
select = dplyr::select
load("results/volcanoplot_all_NormGM_F_ados_unadjBMI_unadjICV_2025-03-26.Rdata")
volcano_main_NormGM_F = volcano_main
rm(volcano_main)

load("results/volcanoplot_all_lobar.vol.GM.adjICV_F_ados_unadjBMI_2025-05-26.Rdata")
volcano_main_lobarVolGM_F = volcano_main
rm(volcano_main)

  load("results/volcanoplot_all_LobarGM_Z_F_ados_unadjBMI_2025-11-25.Rdata")
volcano_main_MTR_GM_F = volcano_main

load("results/volcanoplot_all_NormGM_M_ados_unadjBMI_unadjICV_2025-03-26.Rdata")
volcano_main_NormGM_M = volcano_main
rm(volcano_main)

load("results/volcanoplot_all_lobar.vol.GM.adjICV_M_ados_unadjBMI_2025-05-26.Rdata")
volcano_main_lobarVolGM_M = volcano_main
rm(volcano_main)

load("results/volcanoplot_all_LobarGM_Z_M_ados_unadjBMI_2025-11-25.Rdata")
volcano_main_MTR_GM_M = volcano_main

beta_3brain_F =  volcano_main_lobarVolGM_F$data %>% select(metabolite_id,beta) %>% rename(beta_Vol.GM=beta) %>%
  left_join(volcano_main_NormGM_F$data %>% select(metabolite_id,beta) %>% rename(beta_Norm.GM=beta), 
            join_by(metabolite_id)) %>%
  left_join(volcano_main_MTR_GM_F$data %>% select(metabolite_id,beta) %>% rename(beta_MTR.GM=beta), 
            join_by(metabolite_id)) %>%
  filter(metabolite_id %in% lipids_incl)
  

beta_3brain_M =  volcano_main_lobarVolGM_M$data %>% select(metabolite_id,beta) %>% rename(beta_Vol.GM=beta) %>%
  left_join(volcano_main_NormGM_M$data %>% select(metabolite_id,beta) %>% rename(beta_Norm.GM=beta), 
            join_by(metabolite_id)) %>%
  left_join(volcano_main_MTR_GM_M$data %>% select(metabolite_id,beta) %>% rename(beta_MTR.GM=beta), 
            join_by(metabolite_id)) %>%
  filter(metabolite_id %in% lipids_incl)
...
ggcorr_female = create_ggcorrplot (
  data = beta_3brain_F ,
  sel_colnames = c("beta_Vol.GM","beta_Norm.GM","beta_MTR.GM"),
  new_colnames = new_colnames_beta,
  ID_colname = "metabolite_id"
)
...
ggsave(file.path(opt$output.dir,paste0("brain_corrplot_beta_GM.jpg")), 
       bg = "white",
...
p
ggsave(file.path(opt$output.dir,paste0("brain_scatterplot_beta_GM.jpg")), 
       bg = "white",
       plot = p,
       width = 8.5*3*1.25, 
       height = 4.5*1.5*1.25, 
